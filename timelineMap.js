function init(){
	...
	var text10 = new dataInfoText('<ul><li>From <b>100 AD</b>, a cultural influence from the Roman Empire took place.</li><li>Trading in largely furs and skins in exchange for luxury goods occured.</li><li>Some Scandinavians also served as Roman mercenaries.</li><li>Trade, and adoption of gold and Roman coinange as currency, led to new divergences in wealth in previusly more egalitarian societies. Some of the larger Norse farmers becoming chieftains. They functioned as priests and accepted sacrifices from farmers which were again used to pay soldiers. They would obtain a group of soldiers to protect lands called a hird. Thus they were able to rule an area of several settlements and tribes.</li></ul>');
	...
	//set initial range ...their setWindow function appears not to work well with dates
	//before 1900...trying to work around to get data range early as possible,
	//this yields 190 - 250 AD
	timeline.setWindow({start:'0001', end: '0121',});
	...
	//call move(percentage) on tap of moveLeft and right arrow images
	...
	function makeInfoBox(id, pointTop, pointLeft){
		...
		$("#info").remove();
		$("#titleBox").remove();
		//Create #info with id #info, class .textHolder, with top and left at first aligned with those on 'point'+id.
	...
	function makeNewPoint(id, startYear, endYear, pointTop, pointLeft){		
		...
		//makeHoverTitle on the point
		makeHoverTitle(pointTop, pointLeft);
		/*if point'+id+' is clicked, tapped, or corresponding timeline block is, 		
		makeInfoBox, set the selection to highlight on the timeline (.setSelection
		property takes in an array of id(s)).
		*/
